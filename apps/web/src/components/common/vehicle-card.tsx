...
import { Car, Gauge, Fuel, GitFork } from 'lucide-react';
...
export function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border bg-card shadow-sm transition-all hover:shadow-lg hover:-translate-y-0.5">
      {/* Image */}
      <Link href={`/vehicles/${slug}`} aria-label={`View ${vehicle.year} ${vehicle.make} ${vehicle.model}`} tabIndex={-1}>
        <div className="relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">
          <Car className="h-20 w-20 text-slate-300 transition-transform group-hover:scale-105" aria-hidden="true" />
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex-1">
          <Link href={`/vehicles/${slug}`} className="group/title">
            <h2 className="font-semibold text-foreground leading-tight group-hover/title:text-primary transition-colors">
          ...
          <dl className="mt-3 grid grid-cols-3 gap-2">
            ...
            <div className="flex flex-col items-center rounded-lg bg-muted/50 px-2 py-1.5 text-center">
              ...
              <dd className="text-[10px] font-medium text-muted-foreground leading-tight">{vehicle.fuelType}</dd>
            </div>
            <div className="flex flex-col items-center rounded-lg bg-muted/50 px-2 py-1.5 text-center">
              <GitFork className="mb-0.5 h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
              <dt className="sr-only">Transmission</dt>
              <dd className="text-[10px] font-medium text-muted-foreground leading-tight">
                {vehicle.transmission === 'SEMI_AUTOMATIC' ? 'Semi-Auto' : vehicle.transmission.charAt(0) + vehicle.transmission.slice(1).toLowerCase()}
              </dd>
            </div>
          </dl>
        </div>
